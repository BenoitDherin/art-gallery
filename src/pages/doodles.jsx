...
import { doodlesPathPrefix } from '../constants'
...
function deriveSlug(relativePath) {
  const withoutExtension = relativePath.split('.')[0].replace(doodlesPathPrefix, '')
  return `${withoutExtension}/`
}

export const query = graphql`
  query {
    allFile(filter: {relativePath: {regex: "/^static\/images\/doodles/"}}) {
      nodes {
        id
        childImageSharp {
          fluid {
